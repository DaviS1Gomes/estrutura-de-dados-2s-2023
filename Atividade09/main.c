...
int main(int argc, char* argv[]){
    ...
    double tempoMedioNaoOrdenada = calcularTempoMedioInsercao(lista_nao_ordenada, arquivo);

    fseek(arquivo, 0, SEEK_SET);

    double tempoMedioOrdenada = calcularTempoMedioInsercao(lista_ordenada, arquivo);

    fclose(arquivo);

    printf("Tempo médio de inserção na lista não ordenada: %.6f segundos\n", tempoMedioNaoOrdenada);
